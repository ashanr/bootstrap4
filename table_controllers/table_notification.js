function load_notification_table(text, callBack) {
    var tableData = '';
    $.post("table_models/table_model_notification.php", {table: 'load_notificaiton_table', text: text}, function (e) {
        if (e === undefined || e.length === 0 || e === null) {
            ...
            $.each(e, function (index, qData) {
                ...
                var message = qData.message;
                var short_message = message.substr(0,50);
                tableData += '<tr>';
                tableData += '<td>' + index + '</td>';
                tableData += '<td>' + qData.title + '</td>';
                                
                if(message.length>50){
                     tableData += '<td>' + short_message + '.....</td>';
                }else{
                     tableData += '<td>' + short_message + '</td>';
                }
